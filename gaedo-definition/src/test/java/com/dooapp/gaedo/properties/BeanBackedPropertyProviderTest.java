...
public class BeanBackedPropertyProviderTest {
	private PropertyProvider tested;
	@Before
	public void prepare() {
		tested = new BeanBackedPropertyProvider();
	}
	/**
	 * Test that there are 4 fields (the three known, and the support one
	 */
	@Test
	public void testFieldsCount() {
		Property[] p = tested.get(RootBean.class);
		Assert.assertThat(p.length, Is.is(3));
	}
