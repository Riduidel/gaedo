...
public class IndexBrowser {
	...
	public Vertex browseFor(IndexableGraph database, String objectVertexId, String className, VertexMatcher matcher) {
		...
		try {
			for (Vertex vertex : matchingIterable) {
				String vertexTypeName = null;
				Kind vertexKind = null;
				String vertexId = null;
				try {
					// / BEWARE : order is signifiant here : read id then kind then
					// type for catch clauses to work correctly
					vertexId = matcher.getIdOf(vertex);
					vertexKind = matcher.getKindOf(vertex);
					vertexTypeName = matcher.getTypeOf(vertex);
					// this slow-down is a direct consequence of
					// https://github.com/Riduidel/gaedo/issues/66
					...
					// vertex is clearly not the match we expected so just navigate
					// to next in index
					if (logger.isLoggable(Level.WARNING)) {
						logger.log(Level.WARNING, "your index may be corrupted...\nWhen looking for value=\"" + objectVertexId + "\", that vertex loaded badly", e);
					}
				} catch (UnableToGetVertexTypeException e) {
					if (GraphMappingStrategy.STRING_TYPE.equals(className)) {
						// in that very case, we can use a type-less vertex as our
						// result
			...
			matchingIterable.close();
		}
		return defaultVertex;
	}

	protected CloseableIterable<Vertex> getVerticesWithId(IndexableGraph database, String objectVertexId) {
		return database.getIndex(IndexNames.VERTICES.getIndexName(), Vertex.class).get(Properties.value.name(), objectVertexId);
	}
