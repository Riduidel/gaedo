...
public class EntryScore {
	 ...
	 * @return
	 */
	public EntryScore findBestMatch(Entry<Vertex, Iterable<Property>> entry) {
	 ...
	 * @return
	 */
	private EntryScore findBestMatch(Entry<Vertex, Iterable<Property>> entry, AtomicLong edgesCount, ArrayList<Vertex> verticesToCountEdgesIn, Iterator<Property> pathIterator) {
		if(pathIterator.hasNext()) {
			Property current = pathIterator.next();
			EntryScore matching = findBestMatch(entry, edgesCount, verticesToCountEdgesIn, pathIterator);
			...
			if(matching==this) {
				...
				for(Vertex v : toScan) {
					...
					for(Edge e : edges) {
						verticesToCountEdgesIn.add(e.getInVertex());
					}
				}
				long totalEdgeCount = edgesCount.addAndGet(verticesToCountEdgesIn.size());
			...
			return new EntryScore().withEntry(entry).withEdgesCount(edgesCount.get());
		}
	}
	 ...
	 */
	public EntryScore withEdgesCount(long edgesCount) {
	 ...
	 * @return this object for chaining calls
	 */
	public EntryScore withEntry(Entry<Vertex, Iterable<Property>> entry) {
		this.setEntry(entry);
		return this;
	}
}